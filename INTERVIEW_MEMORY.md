...
**实验方法：** 用同一个 15 条消息的 transcript，分别跑 3 次提取，只改阈值。

| 阈值 | 候选数 | 通过数 | 结果 |
|------|--------|--------|------|
| 0.5 | 22 | 1 | "Pi 测试框架"一条通过，"用户读过 agent-loop.ts"被过滤 |
| 0.35 | 22 | 7 | knowledge + preference + 核心 fact 都通过 |
| 0.2 | 22 | 15 | extension 安装记录等噪音大量通过 |

**结论：** 0.35 是甜点——重要信息不被误杀，噪音不过量。

**面试时怎么说：** "我跑了三轮对照实验，同一个 transcript 用不同阈值。0.5 时最重要的 knowledge 记忆被过滤了——因为短对话里重要的事只说一次，frequency 得分低。0.35 让知识声明能过线，但噪音（extension 安装记录）因为 relevance 和 evidence 得分低还是被过滤。"

#### 适配 2：frequency 跨会话累积（逻辑推导 + 代码验证）

**发现过程：** 最初给 knowledge 类型特殊待遇——"只说一次也给满分 0.24"。但这是 hack，因为 preference/decision/rule 也可能只说一次但很重要。

**问题分析：** 评分公式的 frequency 维度设计初衷就是跨时间累积——OpenClaw 的 transcript 有几百条消息，同一主题自然出现多次。我们的单次 transcript 只有 15 条，同一主题最多出现 1-2 次。**公式没问题，是输入数据规模不对。**

**修复：** 用 `.scores.json` 跟踪每个记忆标题在历次提取中出现的次数。frequency 不再只看单次 transcript，而是：
```
sessionMentions（本次） + historicalHits（历史累积） × 0.15
```

**验证：** `"用户读过 agent-loop.ts"` 第一次被提取时 frequency 得分 0.08，第三次被提取时 0.15——随项目推进自然增长。不需要特殊照顾任何类型。

**面试时怎么说：** "我发现给 knowledge 特殊待遇是 hack——真正的问题是 frequency 维度需要跨会话累积，但我们只看了单次 transcript。改成了从 .scores.json 读历史累积次数。这和 OpenClaw 的设计意图一致——frequency 的价值在于跨时间，不是单次对话里数数。"

#### 适配 3：Deep Sleep LLM 兜底（对照实验）

**实验方法：** 同一批 7 个评分通过的候选，不跑 Deep Sleep vs 跑 Deep Sleep，对比最终保存结果。

| 状态 | 不跑 Deep Sleep | 跑 Deep Sleep |
|------|----------------|--------------|
| "用户已读过 agent-loop.ts" | ✅ 保存 | ✅ 保存 |
| "用户对 Pi 扩展系统有深入理解" | ✅ 保存 | ✅ 保存 |
| "Pi 测试框架用 harness.ts" | ✅ 保存 | ❌ 删除（代码可发现） |
| "Pi 工具接口是 AgentTool" | ✅ 保存 | ❌ 删除（代码可发现） |
| "Pi 不使用 LLM_PROVIDER" | ✅ 保存 | ✅ 保存 |
| "Pi 没有跨 session 记忆" | ✅ 保存 | ✅ 保存 |
| "agentLoop 导入路径" | ✅ 保存 | ❌ 删除（代码可发现） |

**结论：** Deep Sleep 正确识别并删除了 3 条"读源码就能知道"的 fact，保留了真正的知识和偏好。7 条 → 4 条，质量明显提升。

**面试时怎么说：** "我做了对照实验——同一批候选，跑和不跑 Deep Sleep 对比。不跑的话，'Pi 工具接口是 AgentTool' 这种读源码就知道的信息会被保存。跑了 Deep Sleep，LLM 判断'这条代码里能读到，不需要记忆'并正确删除。这验证了确定性评分需要 LLM 审核做互补。"
