...
4. **项目记忆可能过度积累。** 大部分项目只用 Agent 3-5 次就完了，存持久记忆是浪费。解决方案：30 天自动退休 + 全局记忆不受影响。Pi 本身的 `--resume` 恢复 transcript 对短周期项目更合适。

5. **大部分项目不需要项目记忆。** 一个开发者一年碰 10+ 个项目，只有 2-3 个是长期维护的。对短期项目，Pi 的 session transcript 恢复（`--resume`）优于持久记忆。项目记忆只对长期项目有价值。

每个缺点的根因：**不是所有问题都需要在 50 条记忆的规模下解决。** 过度设计比设计不足更危险。
