...
1. **没有语义搜索。** SearchMemory 是全文 grep，搜"认证"找不到标题是"JWT"的记忆。50 条记忆以下浏览分类目录就够了，500 条以上需要向量检索。这是有意延迟的优化。

2. **Deep Sleep 每天多一次 LLM 调用。** 5 阶段管道比纯确定性评分多花约 0.01 元/天（DeepSeek）。对个人开发者可忽略，但如果要降到零成本，可以去掉 Deep Sleep 退回纯评分。

3. **间歇性维护项目可能被 30 天休眠误删。** 一个项目集中改 2 天、放 2 个月、再回来改——30 天后记忆被删，回来时上下文丢了。这是简单规则（30 天 = 删除）换来的零维护成本。

4. **依赖 transcript 质量。** 如果用户连续几天都在调试/闲聊，提取不出有用记忆。系统不会编造记忆（证据验证），但也不会创造不存在的价值。

5. **单机绑定。** 记忆存在 `~/.triple-pi/memory/`，换电脑就没了。当前假设个人开发者只用一台机器。多人协作或换机场景需要同步方案（git 同步 memory 目录或上云存储）。

每个缺点的根因：**在 50 条记忆的规模下，这些问题的影响远小于它们对应的解决方案的复杂度。** 知道什么时候该升级，比一开始就做全更重要。
...
### 困难 6：LLM 自主调用 Memory 工具 —— 实时写入 vs 异步提取

**问题：**
实际测试中发现，当用户说"你能不能调 SaveMemory 工具"时，Agent 就真的调了，存了一条毫无意义的记忆（"调用 SaveMemory 工具的测试记录"）。用户根本没要求记住任何东西，但 LLM 倾向于"展示自己能调"。

这暴露了一个根深层的设计问题：**把记忆写入决策交给 LLM 实时判断，LLM 会过度调用。** 因为 LLM 的优化目标包含"让用户满意"——用户问能不能调，调了就等于证明能调。

**根因分析：**
OpenClaw 的解决方案完全不同——它不让 Agent 在对话中实时决定"什么该记住"。记忆写入发生在后台：
- 对话结束后，Dreaming 系统异步扫描 transcript
- 提取候选记忆（Light Sleep）
- 加权评分决定是否提升为长期记忆
- 只有 grounded snippets（有原文证据的）才能进入长期存储

这是"实时调用"和"异步提取"两种范式的区别。

**解决过程：**
1. 短期修复：增强 tool description，添加反例（"用户只是在测试功能时不要调用"）
2. 中期方案：在执行前检查上下文——用户是否包含"记住"、"别忘了"等明确意图关键词
3. 长期方向：引入异步提取模式，对话时不写记忆，后台定时扫描 transcript 提取候选

**当前选择：** 保留实时调用（简单、可验证），但明确在面试中承认这个 trade-off——"我知道实时调用有过度写入的问题，OpenClaw 用异步提取解决，我当前阶段选择简单方案，会在这个模块的 v2 版本引入类似机制。"

**教训：**
实际测试比代码审查更能发现问题。这个缺陷是写单元测试发现不了的——只有真正和 Agent 对话，输入边界 case，才能看到 LLM 的行为模式。这也验证了 Eval 框架的重要性。

**面试怎么讲：**
> "我遇到一个很有意思的问题——让 Agent 实时判断什么时候该记东西，它倾向于过度调用。这其实不是 bug，是实时调用模式的固有缺陷。OpenClaw 的做法是对话时不写记忆，后台异步扫描 transcript 提取候选。我当前选择了更简单的实时调用方案，但这是我在文档里明确记录的已知 trade-off，v2 会引入异步提取。"

---

### 困难 7：分类体系不完整 —— "用户读过 agent-loop.ts"掉进了裂缝

**问题：**
实际运行提取器后，用户指出一个关键遗漏：他说过"我已经读过 agent-loop.ts 源码"，但这条信息没有被提取。因为这既不是 preference（偏好），也不是 decision（决策），也不是 rule（规则），放到 fact 又显得不太像。它掉进了四个分类的裂缝里。

但这恰恰是最重要的信息——如果 Agent 不知道用户的知识水平，要么重复解释已知内容，要么跳过用户需要的基础。

**解决过程：**
1. 分析 OpenClaw 的分类：它有 `user` 类型专门存用户相关信息
2. 新增第 5 个分类 `knowledge`——用户的知识水平、学习进度
3. 评分调整：knowledge 声明只说一次就够了，给满分 frequency score（0.24），不因低频被过滤
4. 在提取 prompt 里把 knowledge 列为"最重要的分类"

**效果：**
```
修复前：0 条 knowledge
修复后：✅ [knowledge] "用户已读过 agent-loop.ts 源码" (score: 0.44)
         ✅ [knowledge] "用户熟悉 TypeScript 但不太懂 Rust"
```

**面试怎么讲：**
> "我最初设计了四个分类，觉得自己考虑周全了。但实际跑起来发现，'用户读过 agent-loop.ts'这种知识状态声明没被提取。我意识到缺了一个分类——knowledge 类型比其他类型更重要，因为它决定了 Agent 对话的起点。我参考 OpenClaw 的 user 类型加了这个分类，并且因为知识声明只说一次，给了满分 frequency score。"
...
**问题：**
用户指出了一个我没考虑到的问题：他同时做多个项目。关于 Pi 内部细节的记忆（"agentLoop 导出路径是 X"）在做 React 项目时完全没用，却被注入到 system prompt，浪费 token 且稀释关键信息。当前的全局记忆池没有项目隔离。

**解决过程：**
1. 分析：一个开发者通常同时维护 3-5 个项目，每个项目的技术栈、规则、背景完全不同
2. 设计项目识别机制：优先用 `git remote get-url origin` 做唯一标识，fallback 到目录路径哈希
3. 重构存储结构：
   ```
   ~/.triple-pi/memory/
     global/              ← 跨项目（沟通风格、通用偏好）
     github-com-xxx-xxx/  ← 项目 A（Pi 内部细节、extension 选择）
     github-com-yyy-yyy/  ← 项目 B（React 规则、后端 API 规范）
   ```
4. 启动时只加载 global + 当前项目索引。SaveMemory 默认存到当前项目，加 `scope: global` 参数支持全局记忆
5. 从提取器的 prompt 里明确：代码可发现的信息不存为 fact

**效果：**
```
在 Triple-pi 项目：加载 29 条记忆（其中 0 条来自 React 项目）
在 React 项目：   加载 0 条记忆（Pi 内部细节不会污染）
全局（跨项目）：   "用户偏好简洁回复"（所有项目生效）
```

**面试怎么讲：**
> "这个问题的发现过程很有意思——不是我想出来的，是用户问'agent-loop.ts 的细节在做其他项目时有什么用'。我意识到全局记忆池对多项目开发者是反模式。解决方案是用 git remote URL 做项目标识，记忆自动隔离到项目目录。全局和项目记忆分开，启动时只加载相关的。原则：做 React 项目时，Pi 的内部细节不应该占用上下文。"
...
| 版本 | 发现的问题 | 怎么发现的 | 解决方案 |
|------|-----------|-----------|---------|
| v0.1 | 实时 SaveMemory 被 LLM 过度调用 | 实际对话测试 | 改异步提取模式 |
| v0.4 | 评分阈值 0.5 对短对话太严 | 跑 extractor 看到 22→1 | 降到 0.35 |
| v0.5 | LLM 抄 prompt 里的示例 | 看到"JWT 认证"凭空出现 | 去掉所有示例，加 CRITICAL 警告 |
| v0.5 | 存了大量 extension 安装记录 | 用户质疑"配置本身就是记忆" | 加 Permanence Test + 文件可发现测试 |
| v0.6 | "用户读过 agent-loop.ts"没被提取 | 用户发现遗漏 | 新增 knowledge 类型，单次声明满分 |
| v0.7 | fact 类型提取了代码可发现信息 | 用户质疑"Agent 读文件就知道" | fact 加可发现性过滤 |
| v0.8 | 项目记忆混一起，做 React 看到 Pi 内存 | 用户问"做其他项目有用吗" | 项目级隔离：global + per-project |
| v0.9 | fallback 到 local-xxx 不安全 | 调研 OpenClaw 做法 | workspace 路径识别，fail-closed |
| v0.10 | 30 天退休拍脑袋，判断不了短期/长期项目 | 用户追问"你怎么判断" | 删掉固定退休，用项目隔离解决污染 |
| v0.11 | 项目隔离够但退休机制多此一举 | 用户追问"真的不需要退休吗" | 确认：项目隔离已解决核心问题 |
| v0.12 | 固定时间窗口区分不了三种项目类型 | 用户举出 Type A/B/C 场景 | 改 activity-based dormancy（.last-active） |
| v0.13 | 评分公式算不出记忆质量 | 用户指出质量不够 | 加 Deep Sleep（二次 LLM 审核）|
| v0.13 | 30 天足够判断个人项目是否放弃 | 用户说"直接删不用问" | 简化：30天不碰=删除 |

**关键点：13 个版本中 11 个问题的发现来自实际使用和用户质疑，不是提前设计的。** 面试官能分辨"提前设计好所有东西"和"在真实使用中迭代改进"——后者才是工程能力。
