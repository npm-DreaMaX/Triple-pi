...
| v0.5 | LLM 抄 prompt 里的示例 | 看到"JWT 认证"、"迁移 Go"凭空出现 | 去掉所有示例，加 CRITICAL 警告 |
...
| v0.8 | 所有项目记忆混一起，做 React 时看到 Pi 内存 | 用户问"做其他项目有用吗" | 项目级隔离：global + per-project |

**关键点：这些问题中有 5/7 是通过实际使用发现的，不是提前设计出来的。** 面试官能分辨"提前设计好所有东西"和"在真实使用中迭代改进"的区别——后者是真正的工程能力。
